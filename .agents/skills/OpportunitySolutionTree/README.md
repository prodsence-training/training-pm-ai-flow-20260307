...
│
└── examples/
    ├── product-config-example.md
    │   └─ Web Dashboard 配置示例
    │
    └── [你的 product-config-*.md]
        └─ 你的產品配置（學員自行建立）
```
...
複製 `product-config-template.md`，填入你的產品資訊：

```bash
cp references/product-config-template.md product-config-my-product.md
# 編輯 product-config-my-product.md，填入你的產品信息
```
...
```bash
# macOS / Linux
cp product-config-my-product.md ~/.claude/skills/OpportunitySolutionTree/examples/

# Windows
copy product-config-my-product.md %USERPROFILE%\.claude\skills\OpportunitySolutionTree\examples\
```
...
```bash
# macOS / Linux
cp product-config-my-product.md ~/.agents/skills/OpportunitySolutionTree/examples/

# Windows
copy product-config-my-product.md %USERPROFILE%\.agents\skills\OpportunitySolutionTree\examples\
```
...
```
我要用 OST Skill 進行產品探索。
請使用配置文件：product-config-my-product.md
...
# 複製配置到項目目錄
cp product-config-my-product.md \
...
想看完整的配置示例？詳見 `examples/product-config-example.md`（Web Dashboard）

想建立自己的配置？複製 `references/product-config-template.md` 並填入你的產品信息。
